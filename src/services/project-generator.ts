import type { InitOptions, TemplateConfig, GeneratedFile } from '../types.js';
...
export class ProjectGenerator {
  ...
  async scaffold(options: InitOptions): Promise<void> {
    ...
    await fs.writeFile(
      path.join(projectPath, 'shopify.app.toml'),
      shopifyConfig
    );
...
`
  ...
  private getTemplate(templateType: string): TemplateConfig {
...
`,
  ...
  private createPackageJson(options: InitOptions, template: TemplateConfig): Record<string, unknown> {
    return {
  ...
  private createShopifyConfig(options: InitOptions): string {
    return `[app]
name = "${options.projectName}"

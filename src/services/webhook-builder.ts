...
export class WebhookBuilder {
  ...
  private toFileName(topic: string): string {
    return topic.replace('/', '_');
  }

  private generateWebhookHandler(topic: string, context: Record<string, unknown>): string {
  ...
  private generateRetryUtility(): string {
    return [
      "/**",
  ...
  private generateVerification(context: Record<string, unknown>): string {
    const apiVersion = String(context.shopifyApiVersion || '2024-10');
    return [
  ...
  private toHandlerName(topic: string): string {

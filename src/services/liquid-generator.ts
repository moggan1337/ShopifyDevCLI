...
export class LiquidGenerator {
  ...
  private async generateSection(prompt: string, context: Record<string, unknown>): Promise<GeneratedFile[]> {
  ...
  private async generateSnippet(prompt: string, context: Record<string, unknown>): Promise<GeneratedFile[]> {
  ...
  private async generateLayout(prompt: string, context: Record<string, unknown>): Promise<GeneratedFile[]> {
    return [{
  ...
  private async generateComponent(prompt: string, context: Record<string, unknown>): Promise<GeneratedFile[]> {
  ...
  private extractSectionName(prompt: string): string {
  ...
  private extractSnippetName(prompt: string): string {
  ...
  private generateSchema(prompt: string): Record<string, unknown> {
    ...
    if (prompt.toLowerCase().includes('image') || prompt.toLowerCase().includes('hero')) {
      (schema.settings as Array<Record<string, unknown>>).push({
        type: 'image_picker',
        id: 'image',
        label: 'Image',
      });
  ...
  private generateSectionTemplate(name: string, schema: Record<string, unknown>, prompt: string): string {
    const settings = (schema.settings as Array<Record<string, unknown>>).map(s => `    {
      type: "${s.type}",
      id: "${s.id}",
...
`;
  ...
  private generateSnippetTemplate(name: string, prompt: string): string {
    return `{% comment %}
  Snippet: ${name}
  Description: ${prompt.slice(0, 100)}...
{% endcomment %}
...
`;
  ...
  private generateComponentTemplate(name: string, prompt: string): string {
    return `{% comment %}
  Component: ${name}
  Description: ${prompt.slice(0, 100)}...
{% endcomment %}

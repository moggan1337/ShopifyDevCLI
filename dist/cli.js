...
export class McpCli {
        ...
        if (!prompt) {
            const answers = await inquirer.prompt([
                ...
                },
                {
                    ...
                    validate: (input) => input.length > 3 || 'Please provide a description',
                },
            ]);
            options = {
                type: answers.type,
    ...
    async startTunnel(port, provider) {
        if (provider === 'ngrok') {
            try {
    ...
    async login() {
        ...
        // Save to config
        await this.fileManager.writeFile(`${process.env.HOME}/.shopify/store.json`, JSON.stringify({
            store: answers.store,
    ...
    async updateWebhookUrls(baseUrl) {
